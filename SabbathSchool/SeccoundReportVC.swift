...
var entityIdForThirdReport = 0
var nameEntityForThirdReport = ""

class SeccoundReportVC: UIViewController, UITableViewDataSource, UITableViewDelegate, ChartViewDelegate, NVActivityIndicatorViewable {
    ...
    var listEntity: [Seccound_ListEntity_]? {
        didSet{
            tableView.reloadData()
        }
    }
    
    var arrayHistory: [Seccound_ArrayHistory_]?
    
    let operation: String = "getCalendarioRelatorio"
    var entityId = entityIdForSeccoundReport
    ...
    struct Storyboard {
        static let reportCell = "CellSeccoundReport"
        static let segueToThirdReport = "SegueToThirdReport"
    }
    ...
    func configureHeader() {
        ...
        let title = nameEntityForSeccoundReport
    ...
    func getInfo() {
        ...
        Alamofire.request(getInfoEndpoint, method: .post, parameters: getInfoParameters).responseObject { (response: DataResponse<superReport>) in
            ...
            case .success(let data):
                ...
                self.newOneMore = NSEntityDescription.insertNewObject(forEntityName: "Seccound_OneMoreLevel_", into: self.contextObject)
                ...
                if let someList = data.list {
                    for _list in someList {
                        ...
                        let appDel: AppDelegate = (UIApplication.shared.delegate as! AppDelegate)
                        self.contextObject = appDel.managedObjectContext
                        self.newListEntity = NSEntityDescription.insertNewObject(forEntityName: "Seccound_ListEntity_", into: self.contextObject)
                ...
                if let someHistory = data.history {
                    for _history in someHistory {
                        ...
                        let appDel: AppDelegate = (UIApplication.shared.delegate as! AppDelegate)
                        self.contextObject = appDel.managedObjectContext
                        self.newHistory = NSEntityDescription.insertNewObject(forEntityName: "Seccound_ArrayHistory_", into: self.contextObject)
    ...
    func getListCoreData() {
        ...
        let presentRequest:NSFetchRequest<Seccound_ListEntity_> = Seccound_ListEntity_.fetchRequest()
    ...
    func getArrayHistoryCoreData() {
        ...
        let presentRequest:NSFetchRequest<Seccound_ArrayHistory_> = Seccound_ArrayHistory_.fetchRequest()
    ...
    func deleteEntityList() -> Void {
        
        // Initialize Fetch Request
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Seccound_ListEntity_")
    ...
    func deleteArrayHistory() -> Void {
        ...
        // Initialize Fetch Request
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Seccound_ArrayHistory_")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: Storyboard.reportCell, for: indexPath) as! SeccoundReportTVCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if oneMoreLevel  == true {
            ...
            if dateSegue?.entityId_ != nil {
                ...
                entityIdForThirdReport = Int((dateSegue?.entityId_)!)
                nameEntityForThirdReport = (dateSegue?.name_)!
            }
            
            self.performSegue(withIdentifier: Storyboard.segueToThirdReport, sender: nil)
