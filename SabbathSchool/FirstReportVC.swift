...
var entityIdForSeccoundReport = 0
var nameEntityForSeccoundReport = ""

class FirstReportVC: UIViewController, UITableViewDataSource, UITableViewDelegate, ChartViewDelegate, NVActivityIndicatorViewable {
    ...
    var listEntity: [First_ListEntity_]? {
        didSet{
            tableView.reloadData()
        }
    }
    
    var arrayHistory: [First_ArrayHistory_]?
    
    let operation: String = "getCalendarioRelatorio"
    var entityId: Int32 = Int32(entityIdForFirstReport)
    ...
    struct Storyboard {
        static let reportCell = "CellFirstReport"
        static let segueToFirstReport = "SegueToSeccoundReport"
    }
    ...
    func getInfo() {
        ...
        Alamofire.request(getInfoEndpoint, method: .post, parameters: getInfoParameters).responseObject { (response: DataResponse<superReport>) in
            ...
            case .success(let data):
                ...
                self.newOneMore = NSEntityDescription.insertNewObject(forEntityName: "First_OneMoreLevel_", into: self.contextObject)
                ...
                if let someList = data.list {
                    for _list in someList {
                        ...
                        let appDel: AppDelegate = (UIApplication.shared.delegate as! AppDelegate)
                        self.contextObject = appDel.managedObjectContext
                        self.newListEntity = NSEntityDescription.insertNewObject(forEntityName: "First_ListEntity_", into: self.contextObject)
                ...
                if let someHistory = data.history {
                    for _history in someHistory {
                        ...
                        let appDel: AppDelegate = (UIApplication.shared.delegate as! AppDelegate)
                        self.contextObject = appDel.managedObjectContext
                        self.newHistory = NSEntityDescription.insertNewObject(forEntityName: "First_ArrayHistory_", into: self.contextObject)
    ...
    func getListCoreData() {
        ...
        let presentRequest:NSFetchRequest<First_ListEntity_> = First_ListEntity_.fetchRequest()
    ...
    func getArrayHistoryCoreData() {
        ...
        let presentRequest:NSFetchRequest<First_ArrayHistory_> = First_ArrayHistory_.fetchRequest()
    ...
    func deleteEntity() -> Void {
        ...
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "First_ListEntity_")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: Storyboard.reportCell, for: indexPath) as! FirstReportTVCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if oneMoreLevel  == true {
            ...
            if dateSegue?.entityId_ != nil {
                ...
                entityIdForSeccoundReport = Int((dateSegue?.entityId_)!)
                nameEntityForSeccoundReport = (dateSegue?.name_)!
            }
